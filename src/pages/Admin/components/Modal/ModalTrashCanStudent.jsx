...
import {
  ...
  setPageCurrent,
  setPageSize,
} from "@/redux/Trash/studentTrashSlice";
import {
  DeleteOutlined,
  MoreOutlined,
  RedoOutlined,
...
export const ModalTrashCanStudent = ({ open, close }) => {
  ...
  const { studentList, total, pageCurrent, pageSize } = useSelector(
    (state) => state.studentTrash
  );
  ...
  const getStudentListTrash = useQuery({
    ...
    queryFn: async () =>
      trashStudentApi.getTrashAll({ page: pageCurrent, size: pageSize }),
    onSuccess: (res) => {
  ...
  const restoreStudentList = useMutation({
    ...
    mutationFn: async () =>
      await trashStudentApi.restoreStudentList({ ids: selectedRowKeys }),
    onSuccess: async (res) => {
      if (res && res.success === true) {
        setSelectedRowKeys([]);
  ...
  const permanentlyDeletedStudent = useMutation({
    ...
    mutationFn: async (id) =>
      await trashStudentApi.permanentlyDeletedStudent(id),
    onSuccess: (res) => {
      if (res && res.success === true) {
        dispatch(restoreStudent(res.data));
  ...
  const handleChangeRowKey = (e, record) =>
        ...
        <Popover
          ...
          content={
            <Space direction="vertical">
              <ButtonCustom
                loading={restoreStudent.isLoading}
                title="Khôi phục"
                icon={<RedoOutlined />}
