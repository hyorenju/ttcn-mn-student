...
export function ModalFormStudentInfo({ openForm, onChangeClickOpen, dataStudent, onSuccess, disabled }) {
  ...
  const handleCreateStudent = useMutation({
    ...
    onSuccess: (data) => {
      if (data && data.success) {
        ...
        {
          data?.error.errorDetailList?.forEach((e) => message.error(e.message, 2));
        }
      } else if (data?.error?.code === 500) return message.error(data.error.message);
    },
