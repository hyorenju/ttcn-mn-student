...
function FormAuthSuperMod(props) {
  ...
  const handleGetValuesInitPermissions = () => {
    setLoadingSpin(true);
    getAuthorizationAdmin('SUPERMOD')
      .then((res) => {
  ...
  const onFinish = (values) => {
    ...
    updateAuthorizationAdmin({ id: 'SUPERMOD', permissionIds: valueUpdate })
      .then((res) => {
        if (res.data?.success === true) {
          handleGetValuesInitPermissions();
          setLoadingBtn(false);
...
export default FormAuthSuperMod;
