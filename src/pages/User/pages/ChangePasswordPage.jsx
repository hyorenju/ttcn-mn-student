...
function ChangePasswordPage(props) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => visitor.checkTokenChangePassword({ token: tokenChangePassword }),
  });
      ...
      {data?.success === true && (
        <div className='h-[100vh] flex justify-center items-center flex-col bg-gradient-to-tl from-stone-800 via-black to-lime-500'>
          ...
          <div className='backdrop-blur bg-transparent p-16 pt-12 rounded-3xl mt-8 border-2 border-white'>
            <Form size='large' name='normal_login' className='w-[400px]' layout='vertical' onFinish={onFinish}>
              <Form.Item
              ...
              <Form.Item>
                <Button loading={handleSubmit.isLoading} htmlType='submit' className='w-full text-white uppercase mt-8'>
                  Cập nhật mật khẩu
                </Button>
              </Form.Item>
            </Form>
